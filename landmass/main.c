...
#include "clockwork/util/struct/itter.h"
...
static int N_CELLS = 200;
static int N_PLATES = 20;
...
static Surface *newsurface(int w, int h)
{
    Surface *s = malloc(sizeof(Surface));
    s->w = w;
    s->h = h;
    s->pixels = malloc(sizeof(float) * w * h);
}
...
void apply_veronoi(SDL_Surface *s)
{
    ...
    for(j = 0; j < HEIGHT; j++)
        for(i = 0; i < WIDTH; i++){
            float pos[2] = {i + noise2_fracPerlin(i / 32.0f, j / 32.0f, 3) * 8.0f, 
                            j + noise2_fracPerlin((WIDTH - i) / 32.0f, (HEIGHT - j) / 32.0f, 3) * 8.0f};
            float *closest[10];
            void *closestval[10];
            int n = kdtree_closestN(&tree, pos, 5, closestval, closest);
            //float d1 = sqrt(dist(closest[0], pos, 2));
            //float d2 = sqrt(dist(closest[1], pos, 2)) ;
            int id = (*((int*)closestval[0]));
            
            //f = clamp(f, 0, 250);
            //uint32_t color = SDL_MapRGB(s->format, (int) f, (int) f, (int) f);
            int color = COLORS[id&15];
